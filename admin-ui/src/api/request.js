...
/**
 * 后台统一请求封装。
 * - baseURL /api 由 Vite 代理转发到网关 8080
 * - 注入 satoken 请求头（Sa-Token 默认 token 名称）
 * - 统一解析后端 R 结构 { code, message, data, timestamp }
 * - 401/403 统一处理：清理登录态并回到登录页
 */
...
request.interceptors.response.use(
  ...
  (error) => {
    ...
    if (!message) {
      if (status === 403) message = '无权限执行该操作'
      else if (status === 503) message = '服务暂时不可用，请稍后重试'
      else if (status === 404) message = '接口不存在（检查网关路由）'
      else if (error.code === 'ECONNABORTED') message = '请求超时，请稍后重试'
      else message = '网络异常，请检查后端服务是否启动'
    }
